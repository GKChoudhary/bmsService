...
@Component
@Scope("session")
public class EmployeeTypeMasterController implements Serializable {

    *//**
     * serialVersionUID
     *//*
    private static final long serialVersionUID = -7277461597485508675L;
    ...
    public List<EmployeeTypeMst> getAllEmployeeType() {
        ...
        List<EmployeeTypeMst> listEmployeeTypeMstLocal = employeeTypeMstservice.findAllNonDeletedEmployeeTypeMst();
        for (EmployeeTypeMst listEmployeeTypeMstLocal1 : listEmployeeTypeMstLocal) {
            if (!employeeMstL.getEmpTypId().getTypeName().equalsIgnoreCase("Access and Authorization")) {
                listEmployeeTypeMst.add(listEmployeeTypeMstLocal1);
            } else if (!listEmployeeTypeMstLocal1.getTypeName().equalsIgnoreCase("Admin")) {
                listEmployeeTypeMst.add(listEmployeeTypeMstLocal1);
            }
        }
//        listEmployeeTypeMst = employeeTypeMstservice.findAllNonDeletedEmployeeTypeMst();
    ...
    public String checkEmployeeTypeEdit() {
        ...
        if (this.selectEmployeeTypeMst.size() > 0) {
            ...
            RequestContext.getCurrentInstance().execute("editEmpTypeDialog.show();");
